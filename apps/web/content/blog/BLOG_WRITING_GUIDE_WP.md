...
- **다이어그램(PNG)**: 개념·흐름·관계를 보여줘야 할 때. 예: 단계별 플로우차트, 데이터 흐름(전/후 비교), 파일 구조 트리, 아키텍처 레이어.
- **UI 목업(PNG)**: "어떤 화면을 만든다"가 주제일 때 결과물 미리보기 한 장. 실제 앱 화면을 닮은 간단한 목업이면 충분하다.
...
2. **`rsvg-convert`로 PNG 변환** — 명령 예: `rsvg-convert -w 1600 input.svg -o output.png`. 1600px 폭이 Retina 화면에서도 선명하고, 보통 50~100KB 내외로 떨어진다.
3. **WP 미디어 라이브러리에 PNG 업로드** → 업로드 완료 후 이미지 URL 복사 (예: `https://1daymillion.com/wp-content/uploads/2026/05/D01-5-steps-flow.png`).
4. **HTML 작성 단계**에서는 실제 URL을 모르므로 `src="UPLOAD_ME_{파일명}.png"` 형태의 명확한 placeholder를 쓴다. 발행 직전 Find & Replace로 한 번에 교체.
5. `<figure class="figure"><img src="..." alt="..." /><figcaption>...</figcaption></figure>` 형식으로 감싼다.
6. **PNG 생성·확인이 끝나면 작업용 `.svg` 파일은 삭제**한다 — `wp/assets/`에는 WP로 올릴 최종 PNG만 남긴다. 나중에 다이어그램을 수정해야 할 일이 생기면 SVG를 다시 만들면 된다(어차피 에디터상 구조가 단순해서 재작성 비용이 낮다).

배치 예시 — 투두앱 만들기 글:
- 결과물 미리보기 섹션: 완성 화면 UI 목업 PNG
- 실전 5단계 도입부: 단계 흐름 다이어그램 PNG
- 핵심 개념 등장 시점(예: localStorage): "없을 때 vs 있을 때" 비교 PNG
- 진단 섹션: 증상·원인·해결 3열 표
...
- [ ] 공백 제외 3,000자 이상.
- [ ] **시각 자료(표/PNG/코드블록) 3개 이상** 본문에 포함. 최소 1개는 표, 1개는 다이어그램 또는 UI 목업 PNG.
- [ ] 본문에 **인라인 `<svg>` 태그 없음**(WP가 구조 스트립). 작업용 SVG는 `wp/assets/`에 두고 rsvg-convert로 PNG 변환한 뒤 `<img>`로 참조.
