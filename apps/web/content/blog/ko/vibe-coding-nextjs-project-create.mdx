...
터미널에서 각 명령어를 입력해서 버전 번호가 출력되면 준비 완료입니다. 아직 설치가 안 됐다면 [개발환경 세팅 가이드](https://1daymillion.com/vibe-coding-dev-environment-setup/)를 먼저 참고하세요.
...
Node.js가 설치되지 않았거나 PATH에 등록되지 않은 경우입니다. `node --version`을 먼저 확인하세요. 버전이 나오지 않으면 [Node.js 설치 가이드](https://1daymillion.com/vibe-coding-nodejs-install/)를 따라 설치한 뒤 터미널을 재시작합니다.
...
- [바이브코딩으로 포트폴리오 사이트 1시간 만에 만들기](https://1daymillion.com/vibe-coding-portfolio-site-one-hour/)
- [비전공자의 바이브코딩, 첫 번째 벽은 환경 세팅입니다](https://1daymillion.com/vibe-coding-first-wall-environment-setup/)
