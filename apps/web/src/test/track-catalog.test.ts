 ...
 *   - 정적 트랙에 정확히 2개 마일스톤이 배포→AI 수정 순서
...
describe("track catalog — static track", () => {
  ...
  it("exposes exactly 4 tracks with only the real website track enabled", () => {
  ...
  it("static track has exactly 2 milestones in the expected order", () => {
    const milestones = catalog.listMilestones("static");
    expect(milestones).toHaveLength(2);
    expect(milestones.map((m) => m.id)).toEqual(EXPECTED_STATIC_ORDER);
  });

  it("order fields are 1..2 in sequence", () => {
    const orders = staticMilestones.map((m) => m.order);
    expect(orders).toEqual([1, 2]);
  });

  it("unlocks chain is unbroken and terminates at the AI edit milestone", () => {
    const milestones = catalog.listMilestones("static");
  ...
  it("legacy disabled tracks retain the core milestones for existing projects", () => {
